...
- -t, --test_file (stringa, opzionale): Percorso al file CSV contenente i dati di test. Deve includere almeno la colonna Text. Se non fornito, il dataset di training verrà suddiviso automaticamente in base al parametro split_ratio.

- -n, --num_samples (intero, opzionale): Numero di esempi per etichetta da usare per il training. Se impostato a 0, verrà utilizzato l'intero dataset. Utile per il training few-shot.

- -s, --split_ratio (float, opzionale): Percentuale del dataset di training da usare come test se non è fornito un test set. Deve essere un valore compreso tra 0 e 1. Default: 0.3.
